      ...
      <main className="content">
        <header className="topbar">
          <div>
            ...
            <h2 dangerouslySetInnerHTML={{ __html: form.title || 'Form' }} />
          </div>
          ...
          <form onSubmit={handleSubmit} className="form-panel">
            {(form.questions || []).map((question) => (
              <div key={question.id} className="response-question">
                {question.imageSrc && question.type !== 'image' && (
                  <div className="response-image-block">
                    <img src={question.imageSrc} alt={question.title || 'Gambar'} className="response-image-el" />
                  </div>
                )}

                <label className="question-label">
                  <span dangerouslySetInnerHTML={{
                    __html: `${question.title || question.label || ''} ${question.required ? '<strong style="color: #dc2626; margin-left: 2px;">*</strong>' : ''}`
                  }} />
                </label>

                {(question.type === 'text' || question.type === 'sa') && (
                  <input
                    type="text"
                ...
                {(question.type === 'textarea' || question.type === 'paragraph') && (
                  <textarea
                    rows="4"
                    value={answers[question.id] || ''}
                    onChange={(event) => handleChange(question.id, event.target.value)}
                ...
                {(question.type === 'radio' || question.type === 'mc') && (
                ...
                {(question.type === 'checkbox' || question.type === 'cb') && (
                  <div className="choice-group">
                    {(question.options || []).map((option) => (
                      <label key={option.id || option} className="choice-item">
                        <input
                ...
                {question.type === 'image' && (
                  <div className="response-image-block">
                    {question.imageSrc ? (
                      ...
                      <div className="no-image-placeholder">Tidak ada gambar yang diunggah</div>
                    )}
                  </div>
                )}
